...
def translate(root: str, filename: str):
    if filename.endswith(".yaml") or filename.endswith(".yml"):
        extension = ".yaml" if filename.endswith(".yaml") else ".yml"
        update_log(f"[Translating...] {filename}")
        filename = filename.split(extension)[0]

        convert_yaml_to_txt(f"{root}/{filename}{extension}")
        create_yaml_cleanedlist(root, filename)
        translate_txt(f"{root}/{filename}.txt")
        update_log(f"[Finished] {filename}{extension}")
    else:
        update_log(f"[Error: not an YML/YAML file] {filename}")
    ...
    with open("buffer.txt", "r", encoding="utf-8") as file:
        try:
            for line in file:
                ...
                save_leftovers(leftovers, root, filename)
                refine_yaml_cleanedlist(cleaned_yaml)
